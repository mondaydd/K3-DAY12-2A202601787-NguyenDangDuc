...
def ready(store: ConversationStore = Depends(get_store)):
    ...
    """
    # Đang tắt dần → báo không nhận traffic nữa
    if lifecycle.shutting_down:
        return JSONResponse(status_code=503, content={"status": "shutting_down"})
...
def ask(
    ...
    """
    # 1. Rate limit — kiểm tra TRƯỚC khi gọi LLM (tránh mất tiền rồi mới chặn)
    limiter.check(user_id)

    # 2. Cost guard — kiểm tra ngân sách còn đủ không
    guard.check(user_id)

    # 3. Lấy lịch sử hội thoại từ Redis
    history = store.get_history(user_id)

    # 4. Gọi LLM (mock — không tốn tiền thật)
    result = ask_llm(payload.question, history)

    # 5. Lưu câu hỏi và câu trả lời vào lịch sử
    store.append(user_id, "user", payload.question)
    store.append(user_id, "assistant", result["answer"])

    # 6. Ghi nhận chi phí vào Redis
    guard.record(user_id, result["cost_usd"])

    # 7. Log sự kiện (JSON 1 dòng ra stdout)
    log_event(
        "ask_completed",
        user_id=user_id,
